 ...
 * incopro Theme
 */
class incopro_home_clients extends WP_Widget
{
    function __construct(){

        $widget_ops = array('classname' => 'incopro_home_clients','description' => esc_html__( "incopro Client Section That Displays Logos In A Slider" ,'incopro') );
        parent::__construct('incopro_home_clients', esc_html__('[incopro] Client Section For FrontPage','incopro'), $widget_ops);
    }

    function widget($args , $instance) {
    	extract($args);
        $title = isset($instance['title']) && !empty($instance['title']) ? $instance['title'] : __('Our Main Clients','incopro');
                      ...
                      if( $logos['img'] != '' && $logos['link'] != '' ){ ?>
                        <li>
                            <a href="<?php echo $logos['link'][$i]; ?>">
                                <img alt="<?php _e('Logos', 'incopro'); ?>" src="<?php echo $logos['img'][$i]; ?>" />
                            </a>
                        </li><?php
                      }
                    }?>
    ...
    ?>
      ...
      <ul class="client-sortable clone-wrapper"><?php
          ...
          for ( $i = 0; $i < $slider_count; $i++ ): ?>
              <li class="toclone">
                <br>
                  <label class="logo_heading"><b><?php _e( 'Logo #', 'incopro' ); ?><span class="count"><?php echo absint( $i )+1; ?></span></b></label>
                  ...
                  <button id="<?php echo $this->get_field_id('image_src_button').'-'.$i; ?>" class="button button-primary custom_media_button" data-fieldid="<?php echo $this->get_field_id('image_src').'-'.$i; ?>"><?php _e( 'Upload Logo','incopro' ); ?></button>
                  ...
                  <br/><br/>
                  <label><?php _e( 'Link:', 'incopro' ); ?></label>
                  ...
                  <a href="#" class="clone button-primary"><?php _e('Add', 'incopro'); ?></a>
                  <a href="#" class="delete button"><?php _e('Delete', 'incopro'); ?></a>
              </li>
          <?php endfor; ?>
      </ul><?php
