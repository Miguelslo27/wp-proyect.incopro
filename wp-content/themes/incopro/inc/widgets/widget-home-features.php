 ...
 * incopro Theme
 */
class incopro_home_features extends WP_Widget
{
    function __construct(){

        $widget_ops = array('classname' => 'incopro_home_features','description' => esc_html__( "Widget to set Features in Home Section" ,'incopro') );

        parent::__construct('incopro_home_features', esc_html__('[incopro] Features Section For FrontPage','incopro'), $widget_ops);
    ...
    ?>
      ...
      <?php for ( $i=1; $i<4; $i++ ) { ?>
      <br>
      <b><?php echo sprintf( __( "Feature %s", 'incopro' ), $i ); ?></b>
      
      <p><label for="<?php echo $this->get_field_id('title'.$i); ?>"><?php esc_html_e('Title ','incopro') ?></label>
      ...
      <p><label for="<?php echo $this->get_field_id('icon'.$i); ?>"><?php esc_html_e('Icon( Font Awsome ) ','incopro') ?></label><?php
      
        $get_fontawesome_icons = $this->get_fontawesome_icons(); 
        $icon = ( isset( $instance['icon'.$i] ) && $instance['icon'.$i] != '' ) ? esc_html($instance['icon'.$i]): '';?>
        
        <select class="incopro-icon" id="<?php echo $this->get_field_id('icon'.$i); ?>" name="<?php echo $this->get_field_name( 'icon'.$i ); ?>">
            <option value=""><?php _e( 'Select Icon', 'incopro' ); ?></option>
      ...
      <p><label for="<?php echo $this->get_field_id('body_content'.$i); ?>"><?php esc_html_e('Content ','incopro') ?></label>
