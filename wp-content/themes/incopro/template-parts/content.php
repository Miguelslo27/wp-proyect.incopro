	...
	<header class="entry-header nolist">
        ...
        if( has_post_thumbnail() && !is_single() ){ ?>
            <a class="text-center" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
                the_post_thumbnail( 'incopro-featured', array( 'class' => 'mb24')); ?>
            </a><?php
        }
		...
		<?php
            if( !is_single() ){
                ...
                the_content( sprintf(
                    ...
                    wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'incopro' ), array( 'span' => array( 'class' => array() ) ) ),
			...
			wp_link_pages( array(
				'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'incopro' ),
				'after'  => '</div>',
			) );
		?>
	</div><!-- .entry-content -->
